...
def connect_signal_slot(self) -> None:
    ...
    self.ComboModelChoice.currentTextChanged.connect(
        lambda model: gf.filter_model_changed(self, model)
    )
    self.ComboModelChoiceNewEntry.currentTextChanged.connect(
        lambda model: gf.new_entry_model_changed(self, model)
    )
